...
The quick filter displays a checkbox list (click on the icon) where you can select which column to filter by, at the end of this list can appear a checkbox selector to be case sensitive. This option may not appear if `pageable` attribute is setted to false in table component.
